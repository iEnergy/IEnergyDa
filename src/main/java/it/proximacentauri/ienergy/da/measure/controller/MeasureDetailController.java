...
public class MeasureDetailController extends MeasureBaseController {

	final private Logger log = LoggerFactory.getLogger(MeasureDetailController.class);
	...
	protected MyResult queryData(String name, Date start, Date end, TimeAggregation timeAggregation, MeasureAggegation measureAggregation) {
		...
		log.info("get measure (details) for name {} with start: {} end: {} time aggregation: {} measureArregation; {}", name,
				myResult.start, myResult.end, timeAggregation, measureAggregation);

		// load the measure from db
		myResult.result = service.findMeasureDetails(name, myResult.start, myResult.end, timeAggregation, measureAggregation);
		return myResult;
	}

	@Override
	protected Pair<Date, Date> queryTime() {
		return service.findMeasureDetailsTime();
	}
